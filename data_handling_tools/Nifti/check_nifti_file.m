...
elseif (nii.hdr.hist.qform_code > 0) && (nii.hdr.hist.sform_code <= 0)
   if exist('file_workdir', 'var')
      ...
      outFile = fullfile(file_workdir, [remove_extension(name) '.sform_added.nii.gz']);
      add_sform(nifti_file, outFile);
   else
      fprintf('\nOnly qform matrix is defined in file %s. You can run add_sform to add sform matrix to the nifti file.', ...
         escape_filename(nifti_file));
      outFile = [];
   end
   fileFlag = 2;
