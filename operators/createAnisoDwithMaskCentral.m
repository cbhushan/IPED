...
function [D,Dp] = createAnisoDwithMaskCentral(N1,N2,N3,msk,anisoWt,resWt)
% Generates sparse matrix operator D for an image of dimensions N1xN2xN3 such that it computes
% weighted central difference only using voxel inside the input mask i.e. boundary voxels (of mask or
% volume) for which the "forward" & "backward" voxel lies outside mask/volume is not used.
%
% Similar to createAnisoDwithMask(), but uses central difference instread of forward difference.
%
...
% only keep indices which have "forward" & "backward" voxel inside mask
Lia_forw = ismember(i+ind_incr, msk_ind);
Lia_back = ismember(i-ind_incr, msk_ind);
i = i(Lia_forw & Lia_back);

s = wt(i)/2; % division by 2 because of central difference
j = [i+ind_incr; i-ind_incr];
i = [i; i];
s = [s; -s];
D = sparse(i,j,s,m,m);
clearvars -except D
