...
function [mask, thresh] = maskHeadPseudoNcuts(data_file, output_mask_file, if_aggresive)
...
% dataN = dataN(1:5:end, 1:5:end, 1:5:end);

msk_sz = 5;
n_parts = 4;

% Ncuts based segmentation
[gy, gx, gz] = gradient(dataN, data.hdr.dime.pixdim(2), data.hdr.dime.pixdim(3), data.hdr.dime.pixdim(4));
g_mag = sqrt(gy.^2 + gx.^2 + gz.^2);
sgma_itensity = mean(g_mag(:)); 
sgma_spatial = 0.75*msk_sz;
clear gy gx gz g_mag

[ii, jj, dd] = imgNbrMap3D(size(dataN), msk_sz);
pix_diff = (dataN(ii) - dataN(jj)).^2;
wts = (pix_diff/(sgma_itensity*sgma_itensity)) + (dd.^2/(sgma_spatial*sgma_spatial));
clear pix_diff ddsudo 

W = sparse(ii, jj, exp(-0.5 * wts));
clear ii jj wts

[NcutDiscrete] = ncutW(W, n_parts);
